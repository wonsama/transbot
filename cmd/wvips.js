...
const STEEM_TRANS_VIPS = process.env.STEEM_TRANS_VIPS||'';
const STEEM_TRANS_VIPS_AUTHOR = process.env.STEEM_TRANS_VIPS_AUTHOR||'';
const STEEM_KEY_POSTING = process.env.STEEM_KEY_POSTING;	// STEEM_TRANS_FRIENDS_AUTHOR 포스팅키 아니면 위임자의 포스팅 키


const DEFAULT_TAG = 'kr-vips';  // 1개만 설정 가능 - category로도 사용되기 때문
...
fn.name = removeSpace(STEEM_TRANS_VIPS).split(',');
...
fn.command = async (item) =>{
	...
	let err;
	let account = STEEM_TRANS_VIPS_AUTHOR;
  ...
  if(!err && cont && cont.id==0){
    ...
    let title = `${today} VIPS의 새소식 알림`;
    let body = `### VIPS의 새글 정보를 공유합니다.\n\n자세한 내용은 댓글을 참조 바랍니다.`;
    let permlink = `${account}-${today}`;
    permlink = permlink.replace(/\./gi, '-'); // permlink에는 .이 포함되면 안됨에 유의
